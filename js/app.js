...
// Wallet session cache (so taps can trigger wallet popups reliably)
...
async function initWalletSession() {
  if (__walletSession.ready) return __walletSession;
...
function withTimeout(promise, timeoutMs) {
  ...
  const timeout = new Promise((_, reject) => {
    t = window.setTimeout(() => reject(new Error("Timed out waiting for wallet/chain response")), timeoutMs);
  });
...
async function walletSendCallsTap({ counter }) {
  // Important: wallet popups can stop appearing if the request is made after an await.
  // So we warm up the wallet session separately, then send calls using cached session values.
  const { provider, from, ready, paymasterSupported } = __walletSession.ready
    ? __walletSession
  ...
  // Call wallet_sendCalls immediately (no extra awaits before this line)
...
async function processTapQueue() {
  ...
  tapSending = true;
  try {
    // Ensure the wallet session is initialized.
    // If initialization required a prompt, we ask the user to tap again.
    if (!__walletSession.ready) {
      ...
      setTapHint(`Ready — tap to confirm (${tapQueue} queued)`);
      return;
    }
    ...
    const res = await withTimeout(walletSendCallsTap({ counter: nextCounter }), 45000);
    ...
    // Keep tapQueue as-is so user can retry by tapping again.
